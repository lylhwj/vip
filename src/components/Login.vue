    ...
    <div class="w-login">
      ...
      <div v-if="isLogin&&!loginIsQr">
        <div v-if="!loginPhoneMethod" class="login_box">
          <h3>账号登录<a href="javascipt:;" @click="loginPhoneMethod=true">手机快捷登录&nbsp;</a></h3>
          <p class="tip_error" v-if="error">{{errorMsg}}</p>
          <el-input class="mt20 inp_login" v-model="loginForm.login" @keyup.enter.native="login" @change="error=false"
            placeholder="手机/边乐云账号"><i slot="prefix" class="el-input__icon el-icon-user"></i></el-input>
          <p style="position:relative;">
            <el-input class="mt20 inp_login" :type="passwordType4" v-model="loginForm.password" @keyup.enter.native="login"
        ...
        <div v-if="loginPhoneMethod" class="login_box">
          ...
          <el-button :type="phoneChecked?'primary':'info'" v-if="isRegister" class="btn_login">注册并登录</el-button>
      ...
      <div v-if="!isLogin" class="login_box reg_pc">
        <h3>
          ...
          <!-- <a href="javascript:;" @click="regMethods(2)" :class="[regIndex==2?'hover':'']">快速注册</a> -->
          </p>
        </h3>
        <p class="tip_error" v-if="registerError">{{registerErrorMsg}}<a v-if="registerErrorMsg=='该手机号已注册'" class="fr" href="javascript:;" style="color:#00a8ff;" @click="switchLogin();loginPhoneMethod=true">点击切换到手机快捷登录</a></p>
        ...
        <div class="mt20 mb20  ">
          <el-checkbox v-model="registerChecked" check="false" style="margin-right:0;"></el-checkbox> 已阅读并同意
          <a target="_blank" href="http://vip.bianyue.cn/Agreementuser">《用户协议》</a><a target="_blank" href="http://vip.bianyue.cn/AgreementPrivacy">《隐私政策》</a>
  ...
  methods: {
          ...
          this.getUser({
            callback: () => {
              if (!this.userInfo.mobile) {
                this.$emit("goBindPhone", this.formData);
              } else {
                layer.msg(
                  "登录成功",
                  {
    ...
    register() {
      ...
      if (!this.loginForm.registerPhone && this.regIndex == 0) {
        ...
        this.regIndex == 2 &&
        this.loginForm.confirmPassword !== this.loginForm.pwd
      ) {
        this.registerError = true;
    ...
    changePwd(type) {
      ...
      if (type == 1 && !this.loginForm.pwd) {
        ...
        type == 2 &&
        this.loginForm.pwd !== this.loginForm.confirmPassword
      ) {
        this.registerError = true;
        this.registerErrorMsg = "两次密码不一致";
      }
    ...
    getCode() {
      ...
      if (!this.loginForm.loginPhone) {
        ...
        return false;
      } else if (this.loginCodeMsg!=='获取动态密码' ){
        layer.msg("请稍等之后发送", {
          icon: 2
        });
      } else {
    ...
    getRegisterCode() {
      ...
      if (!this.loginForm.registerPhone) {
        ...
        this.registerErrorMsg = "请设置8位以上数字及字母组合的密码";
        return false;
      } else if (this.registerCodeMsg !== "获取验证码") {
        layer.msg("请稍等之后发送", {
          icon: 2
        }); 
      } else {
...
.select_login .el-select .el-input {
  width: 70px;
}

.select_login .el-input-group__prepend {
  border-radius: 0;
  border-color: #c9c9c9;
  background-color: #fff;
}

.select_login .el-input--suffix .el-input__inner {
  padding-right: 20px;
  padding-left: 0;
  text-align: center;
}

.select_login .el-input__inner {
  border-radius: 0;
  border-color: #c9c9c9;
}
...
.inp_yzm .el-input__inner {
  border-radius: 0;
  border-color: #c9c9c9;
}

.inp_yzm .el-button--info {
  float: right;
  width: 126px;
  border-radius: 0;
}

.inp_yzm .el-button--primary {
  float: right;
  width: 126px;
  border-radius: 0;
}
...
.qrcode .img-warpper img {
  cursor: pointer;
  width: 100%;
  height: 100%;
  margin: 0;
}
.w-login .movie_random {
  position: absolute;
  top: 165px;
  left: 50px;
  width: 310px;
...
.eye_close {
  ...
  width: 20px;
  height: 20px;
  background: url(../assets/login/eye.svg);
}
.eye_close {
